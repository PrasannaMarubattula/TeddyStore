(function( $ ) {
	...
	$.Shop.prototype = {
		init: function() {
			...
			this.cartPrefix = "Teddy-"; // Prefix string to be prepended to the cart's name in the session storage
			...
			this.$payform = this.$element.find( "#pay-form" ); // Pay form
			...
			this.payCurrency = "INR"; // Pay currency code
			this.payBusinessEmail = "yourbusiness@email.com"; // Your Business Pay account email address
			//this.payURL = "https://www.sandbox.pay.com/cgi-bin/webscr"; // The URL of the Pay form
		...
		// Appends the required hidden values to the Pay form before submitting
		
		populatePayForm: function() {
			var self = this;
			if( self.$payForm.length ) {
				var $form = self.$payForm;
				...
				$form.attr( "action", self.payURL );
				$form.find( "input[name='business']" ).val( self.payBusinessEmail );
				$form.find( "input[name='currency_code']" ).val( self.payCurrency );
